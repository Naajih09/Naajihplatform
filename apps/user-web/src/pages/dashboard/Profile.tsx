...
const Profile = () => {
  ...
  const profileAvatar =
    formData.avatarUrl || profile.avatarUrl || user.avatarUrl || "";
  const roleLabel =
  ...
  const portfolioHighlights =
    user.role === "ENTREPRENEUR"
      ...
      : [
          ...
          {
            label: "Focus",
            value: profile.focusIndustries?.length
              ? profile.focusIndustries.join(", ")
              : "General",
          },
        ];
  const profileStats = [
  ...
  // Styles
  const inputStyle =
    "w-full p-3 bg-slate-50 dark:bg-[#151518] border border-slate-300 dark:border-gray-700 rounded-xl text-slate-900 dark:text-white focus:border-primary focus:ring-1 focus:ring-primary focus:outline-none transition-all placeholder:text-gray-400";
  const labelStyle =
    "block text-xs font-bold text-slate-500 dark:text-gray-400 uppercase tracking-wider mb-2";
      ...
      <div className="mb-6 flex items-center justify-between sm:mb-8">
        <div>
          ...
          <p className="text-slate-500 dark:text-gray-400 mt-1">
            Manage your public identity and business details.
          </p>
        </div>
      </div>
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm backdrop-blur-md dark:border-white/10 dark:bg-[#1d1f23]/50 sm:p-6">
            ...
            <div className="flex flex-wrap gap-2">
              {(focusTags.length > 0 ? focusTags : ["General"]).map((tag) => (
                <span
                  key={tag}
                  className="px-3 py-1 bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-full text-xs text-slate-600 dark:text-white/80"
                >
                  #{tag}
                </span>
              ))}
            ...
            <div className="relative z-10 flex flex-col items-center gap-6 text-center md:flex-row md:items-start md:gap-8 md:text-left">
              ...
              <div>
                ...
                <h1 className="text-3xl font-extrabold tracking-tight text-white mb-2">
                  {getProfile().firstName || "Update"}{" "}
                  {getProfile().lastName || "Name"}
                </h1>
                ...
                <div className="flex flex-wrap justify-center md:justify-start gap-4 text-gray-400 text-sm">
                  <span className="flex items-center gap-1">
                    <MapPin size={16} /> {getProfile().location || "Nigeria"}
                  </span>
          ...
          {isEditing ? (
                  ...
                  <div className="flex flex-col md:flex-row items-start md:items-center gap-4 rounded-xl border border-dashed border-slate-300 dark:border-gray-700 bg-slate-50 dark:bg-[#151518] p-4">
                    ...
                    <div className="flex-1 space-y-2">
                      <p className="text-sm text-slate-600 dark:text-white/70">
                        Add a clear profile picture so your portfolio feels more
            ...
            <div className="grid grid-cols-2 gap-3 sm:gap-4 md:grid-cols-4">
              {profileStats.map((stat) => (
                <div
                  key={stat.l}
                  className="bg-white dark:bg-[#1d1f23]/50 border border-slate-200 dark:border-white/10 rounded-xl p-4 text-center shadow-sm"
                >
                  <span className="text-slate-500 dark:text-[#abb89d] text-[10px] font-bold uppercase">
                    {stat.l}
                  </span>
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-[#1d1f23] sm:p-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {portfolioHighlights.map((item) => (
                  ...
                  >
                    <p className="text-[10px] uppercase tracking-[0.25em] text-slate-500 dark:text-white/40 font-bold">
                      {item.label}
                    </p>
