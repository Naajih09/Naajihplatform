...
const AcademyProgramDetail = () => {
  ...
  const [lessonCsvFiles, setLessonCsvFiles] = useState<
  ...
  const [lessonImportErrors, setLessonImportErrors] = useState<
    Record<string, string[]>
  >({});
  const [taskImportErrors, setTaskImportErrors] = useState<
    Record<string, string[]>
  >({});
  const [moduleImportErrors, setModuleImportErrors] = useState<string[]>([]);
  const [lessonPreviewHeaders, setLessonPreviewHeaders] = useState<
    Record<string, string[]>
  >({});
  const [lessonPreviewRows, setLessonPreviewRows] = useState<
    Record<string, string[][]>
  >({});
  const [taskPreviewHeaders, setTaskPreviewHeaders] = useState<
    Record<string, string[]>
  >({});
  const [taskPreviewRows, setTaskPreviewRows] = useState<
    Record<string, string[][]>
  >({});
  ...
  const handleImportLessons = async (moduleId: string) => {
    ...
    try {
      ...
      const res = await api.post(
        `/academy/admin/modules/${moduleId}/lessons/import`,
        formData,
        {
  ...
  const handleImportTasks = async (moduleId: string) => {
    ...
    try {
      ...
      formData.append("file", file);
      const res = await api.post(
        `/academy/admin/modules/${moduleId}/tasks/import`,
        formData,
        {
          headers: { "Content-Type": "multipart/form-data" },
        },
  ...
  const handleImportModules = async () => {
    ...
    try {
      ...
      const res = await api.post(
        `/academy/admin/programs/${program.id}/modules/import`,
        formData,
        {
          headers: { "Content-Type": "multipart/form-data" },
        },
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="admin-surface rounded-2xl p-6 space-y-4">
        <h2 className="text-lg font-bold flex items-center gap-2">
          <Upload size={18} className="text-primary" /> Bulk Import Modules
          (CSV)
        </h2>
        <div className="flex flex-col md:flex-row md:items-center gap-3">
          <input
            ...
            accept=".csv"
            onChange={(event) =>
              handleModuleFileChange(event.target.files?.[0])
            }
      ...
      <div className="space-y-6">
        {program.modules?.map((module: any) => (
          <div
            key={module.id}
            className="admin-surface-muted rounded-2xl p-6 space-y-6"
          >
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-gray-500 flex items-center gap-1">
                  ...
                  Unlocks{" "}
                  {module.unlockDate
                    ? new Date(module.unlockDate).toLocaleDateString("en-NG")
                    : "Anytime"}
                </p>
              </div>
            </div>
            ...
            <div className="admin-surface rounded-xl p-4 space-y-3">
              <h4 className="text-xs font-bold text-slate-500 dark:text-gray-400 uppercase">
                Edit Module
              </h4>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                ...
                <input
                  ...
                  value={
                    ...
                    (module.unlockDate
                      ? new Date(module.unlockDate).toISOString().slice(0, 10)
                      : "")
                  }
                  onChange={(event) =>
                    setModuleEdits((prev) => ({
              ...
              <div className="space-y-3">
                ...
                {module.lessons?.length ? (
                  module.lessons.map((lesson: any) => (
                    <div
                      key={lesson.id}
                      className="admin-surface rounded-lg p-3"
                    >
                      <div className="space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <input
                            value={
                              lessonEdits[lesson.id]?.contentType ??
                              lesson.contentType
                            }
                            onChange={(event) =>
                              setLessonEdits((prev) => ({
                          ...
                          <input
                            type="number"
                            value={
                              lessonEdits[lesson.id]?.duration ??
                              lesson.duration
                            }
                            onChange={(event) =>
                              setLessonEdits((prev) => ({
                        ...
                        <input
                          value={
                            lessonEdits[lesson.id]?.videoUrl ??
                            lesson.videoUrl ??
                            ""
                          }
                          onChange={(event) =>
                ...
                <div className="admin-surface-muted rounded-lg p-3 space-y-2">
                  <h5 className="text-xs font-bold text-slate-500 dark:text-gray-400">
                  ...
                  {lessonPreviewHeaders[module.id]?.length > 0 && (
                    <div className="admin-surface rounded-lg p-2 text-[10px] text-slate-700 dark:text-gray-300">
                      <div className="font-bold text-slate-500 dark:text-gray-400 mb-1">
                        Preview
                      </div>
                      <div className="overflow-x-auto">
                        <table className="min-w-full text-[10px]">
                          ...
                          <tbody>
                            {(lessonPreviewRows[module.id] || []).map(
                              (row, index) => (
                                <tr key={`${row.join("-")}-${index}`}>
                                  {row.map((cell, cellIndex) => (
                                    <td
