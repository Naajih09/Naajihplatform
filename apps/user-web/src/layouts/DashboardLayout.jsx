...
import { LayoutGrid, Briefcase, MessageSquare, User, Settings, LogOut, Menu, X, Bell } from 'lucide-react';
    ...
    <div className="flex h-screen bg-brand-bg font-sans overflow-hidden relative">
      ...
      `}>
        <div className="p-6 flex justify-between items-center">
          <div className="text-2xl font-bold text-brand-blue">Naajih<span className="text-brand-gold">Biz</span>.</div>
        ...
        <div className="px-4 py-2">
          <div className="p-4 bg-blue-50 rounded-xl mb-6">
            <p className="text-xs text-gray-500 uppercase font-bold">Current Role</p>
            <p className="font-bold text-brand-blue text-sm">{user.role || 'Guest'}</p>
          </div>
        </div>
        ...
        <div className="p-4 border-t border-gray-100">
           <Link to="/dashboard/settings" className="flex items-center gap-3 px-4 py-3 text-gray-600 hover:bg-gray-100 rounded-lg">
             <Settings size={20} /> Settings
           </Link>
           <button onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 text-red-500 hover:bg-red-50 rounded-lg mt-1">
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden w-full relative z-0">
        <header className="h-16 bg-white border-b border-gray-200 flex justify-between items-center px-4 md:px-8 shrink-0">
          ...
          <h2 className="text-lg font-bold text-brand-dark hidden md:block">Dashboard</h2>
