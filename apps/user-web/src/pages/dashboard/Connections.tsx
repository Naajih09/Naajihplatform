  ...
  const fetchData = async () => {
    try {
      ...
      const resPending = await fetch(
        `${API_BASE}/connections/pending/${user.id}`,
        {
          headers: authHeaders,
        },
      ).then((r) => r.json());
      const resFriends = await fetch(
        `${API_BASE}/connections/user/${user.id}`,
        {
          headers: authHeaders,
        },
      ).then((r) => r.json());
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {pending.map((req) => (
                  <div
                    key={req.id}
                    className="bg-white dark:bg-[#151518] border border-slate-200 dark:border-gray-800 p-4 rounded-xl flex items-center gap-4 shadow-sm"
                  >
                    <div className="size-12 bg-slate-200 dark:bg-gray-800 rounded-full flex items-center justify-center font-bold text-lg text-slate-600 dark:text-gray-400 overflow-hidden">
                      {req.sender?.entrepreneurProfile?.avatarUrl ||
                      req.sender?.investorProfile?.avatarUrl ? (
                        <img
                          src={
                            req.sender?.entrepreneurProfile?.avatarUrl ||
                            req.sender?.investorProfile?.avatarUrl
                          ...
                          alt={
                            req.sender?.entrepreneurProfile?.firstName ||
                            req.sender?.investorProfile?.firstName ||
                            "User"
                          }
                    ...
                    <div className="flex-1">
                      <h4 className="font-bold text-slate-900 dark:text-white">
                        {req.sender?.entrepreneurProfile?.firstName || "User"}
                      </h4>
                    ...
                    <div className="flex gap-2">
                      <button
                        ...
                        className="p-2 bg-primary/10 text-primary rounded-lg hover:bg-primary hover:text-black transition-all disabled:opacity-50"
                        aria-label="Accept"
                      >
            ...
            {friends.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {friends.map((conn) => {
                  ...
                  const profile =
                    other.entrepreneurProfile || other.investorProfile || {};
                  return (
                    <div
                      key={conn.id}
