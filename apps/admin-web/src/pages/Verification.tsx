...
const verificationStatusOptions: VerificationStatusFilter[] = [
  "PENDING",
  "APPROVED",
  "REJECTED",
];
...
const Verification = () => {
  ...
  const fetchRequests = useCallback(async () => {
    try {
      ...
      const res = await api.get(
        `/verification/admin/pending?${params.toString()}`,
      );
      const data = res.data;
      const list =
  ...
  const executeAction = async (
    ...
    try {
      await api.patch(`/verification/admin/${id}`, {
        status,
    ...
    <div className="max-w-6xl mx-auto pb-20 relative">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-8">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1 dark:text-gray-500">
            Review pending, approved, and rejected verification requests.
          </p>
        </div>
      ...
      {loading ? (
        <div className="text-slate-500 dark:text-gray-400 text-center py-20">
          <Loader2 className="animate-spin inline mr-2" /> Loading queue...
        </div>
      ) : requests.length === 0 ? (
              ...
              {requests.map((req) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      <span className="px-2 py-1 bg-blue-500/10 text-blue-400 text-xs rounded font-bold">
                        {req.user?.role}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4 text-right flex justify-end gap-2">
                      <button
                        ...
                        className="px-3 py-1.5 bg-green-600/20 text-green-500 border border-green-600/30 rounded hover:bg-green-600 hover:text-white flex items-center gap-1 transition-all"
                      >
                        <CheckCircle size={14} /> Approve
                      </button>
                      <button
                        onClick={() =>
                          setRejectModal({ show: true, id: req.id })
                        }
                        className="px-3 py-1.5 bg-red-600/20 text-red-500 border border-red-600/30 rounded hover:bg-red-600 hover:text-white flex items-center gap-1 transition-all"
                      >
