...
const PitchesList = () => {
  ...
  const handleStatusUpdate = async (
    ...
    try {
      ...
      setPitches(
        pitches.map((p) =>
          p.id === id
            ? { ...p, status: newStatus, rejectionReason: reason || null }
            : p,
        ),
      );
  ...
  const categories = Array.from(
    new Set(pitches.map((p) => p.category || p.sector).filter(Boolean)),
  );
  ...
  const getStatusBadge = (status: string = "PENDING") => {
    switch (status) {
      case "APPROVED":
        return (
          <span className="bg-green-500/10 text-green-500 border border-green-500/20 text-xs px-2 py-1 rounded font-bold flex items-center gap-1">
            <CheckCircle size={12} /> APPROVED
          </span>
        );
      case "REJECTED":
        return (
          <span className="bg-red-500/10 text-red-500 border border-red-500/20 text-xs px-2 py-1 rounded font-bold flex items-center gap-1">
            <XCircle size={12} /> REJECTED
          </span>
        );
      default:
        return (
          <span className="bg-amber-500/10 text-amber-500 border border-amber-500/20 text-xs px-2 py-1 rounded font-bold flex items-center gap-1">
  ...
  const getImpliedValuation = (pitch: any) => {
    ...
    const equity = Number(
      String(getPitchEquity(pitch) || "").replace(/,/g, ""),
    );
    ...
    <div className="max-w-6xl mx-auto space-y-8 pb-20 relative">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          <div className="relative flex-grow md:flex-grow-0">
            <Search
              className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-500 dark:text-gray-500"
              size={16}
            />
            <input
              type="text"
      ...
      {loading ? (
        <div className="text-center py-20 text-slate-500 dark:text-gray-400">
          <Loader2 className="animate-spin inline mr-2" /> Loading Pitches...
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {pitches.length === 0 ? (
            <div className="col-span-full admin-surface">
              <EmptyState
                title={
                  hasFilters ? "No pitches match this view" : "No pitches yet"
                }
                description={
                  hasFilters
            ...
            pitches.map((pitch) => {
              ...
              return (
                <div
                  key={pitch.id}
                  className="admin-surface p-6 rounded-xl relative flex flex-col h-full hover:border-slate-300 dark:hover:border-white/20 transition-colors"
                >
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      {getStatusBadge(status)}
                      <h3
                        className="text-xl font-bold text-slate-900 dark:text-white mt-3 line-clamp-1"
                        title={pitch.title}
                      >
                        {pitch.title}
                      </h3>
                      <p className="text-primary text-xs font-bold uppercase mt-1">
                        {pitch.category || pitch.sector || "Uncategorized"}
                      </p>
                  ...
                  <div className="admin-surface-muted p-3 rounded-lg mb-4 flex justify-between items-center">
                    <div>
                      <p className="text-[10px] text-slate-500 dark:text-gray-500 uppercase font-bold">
                        Funding Ask
                      </p>
                      <p className="text-slate-900 dark:text-white font-bold">
                        NGN {ask.toLocaleString()}
                      </p>
                    </div>
                  ...
                  <div className="flex gap-2 mt-auto">
                    <button
                      ...
                      className="flex-1 py-2 admin-button-secondary rounded-lg font-bold text-xs transition-all flex items-center justify-center gap-2"
                    >
                      <Eye size={14} /> Details
                    </button>
                    {status === "PENDING" && (
                      <button
                        ...
                        className="flex-1 py-2 bg-primary/20 text-primary border border-primary/30 rounded-lg font-bold text-xs hover:bg-primary hover:text-black transition-all"
                      >
                        Approve
                      </button>
                    )}
          ...
          <div className="bg-white border border-slate-200 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto p-6 relative dark:bg-[#1d1d20] dark:border-white/10">
            ...
            <div className="mb-6 border-b border-slate-200 dark:border-white/10 pb-4 pr-8">
              ...
              <p className="text-slate-600 dark:text-gray-400 mt-2">
                {selectedPitch.tagline}
              </p>
            </div>

            <div className="space-y-6">
              <div>
                <h4 className="text-slate-900 dark:text-white font-bold mb-2 uppercase text-xs tracking-wider">
                  Full Description
                </h4>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div className="admin-surface-muted p-3 rounded">
                  <p className="text-slate-500 dark:text-gray-500 text-[10px] uppercase font-bold mb-1">
                    Funding Ask
                  </p>
                  <p className="text-slate-900 dark:text-white font-bold">
                    NGN{" "}
                    {parseInt(selectedPitch.fundingAsk || "0").toLocaleString()}
                  </p>
                </div>
                <div className="admin-surface-muted p-3 rounded">
                  <p className="text-slate-500 dark:text-gray-500 text-[10px] uppercase font-bold mb-1">
                  ...
                  <p className="text-slate-900 dark:text-white font-bold">
                    {getPitchEquity(selectedPitch) || "N/A"}%
                  </p>
                </div>
                <div className="admin-surface-muted p-3 rounded">
                  <p className="text-slate-500 dark:text-gray-500 text-[10px] uppercase font-bold mb-1">
              ...
              {selectedPitch.businessModel && (
                <div>
                  ...
                  <p className="text-slate-700 dark:text-gray-300 text-sm admin-surface-muted p-4 rounded">
                    {selectedPitch.businessModel}
                  </p>
                </div>
              )}
              ...
              {selectedPitch.status === "REJECTED" &&
                selectedPitch.rejectionReason && (
                  <div>
                    <h4 className="text-red-500 font-bold mb-2 uppercase text-xs tracking-wider">
                      Rejection Reason
                    </h4>
                    <p className="text-slate-700 dark:text-gray-300 text-sm admin-surface-muted p-4 rounded border border-red-500/20">
                      {selectedPitch.rejectionReason}
                    </p>
                  </div>
                )}
              ...
              <div className="flex justify-end gap-3 pt-6 border-t border-slate-200 dark:border-white/10 mt-8">
                {selectedPitch.status !== "APPROVED" && (
                  <button
                    onClick={() =>
                      handleStatusUpdate(selectedPitch.id, "APPROVED")
                    }
                    className="px-6 py-2 bg-green-600 text-white rounded font-bold text-sm hover:bg-green-700"
                  >
                    Approve Pitch
                  </button>
                )}
                {selectedPitch.status !== "REJECTED" && (
                  <button
                    onClick={() => openRejectModal(selectedPitch)}
                    className="px-6 py-2 border border-red-600/50 text-red-500 rounded font-bold text-sm hover:bg-red-900/30"
                  >
                    Reject Pitch
                  </button>
                )}
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[60] p-4">
          <div className="bg-white border border-slate-200 rounded-xl w-full max-w-md p-6 dark:bg-[#1d1d20] dark:border-white/10">
            ...
            <p className="text-sm text-slate-500 dark:text-gray-400 mb-4">
              Explain what the founder needs to fix. The pitch will stay in the
              system as rejected.
            </p>
            <textarea
