...
const DashboardLayout = ({ children }: DashboardLayoutProps) => {
  ...
  const welcomeKey =
    user.id || user.email
      ? `naajihbiz:welcome-seen:${user.id || user.email}`
      : "";
  ...
  useEffect(() => {
    ...
    const currentUser =
      authUser || (userString ? JSON.parse(userString) : null);
  ...
  return (
    <div className="flex h-[100dvh] bg-[#f8fafc] dark:bg-[#111113] text-slate-900 dark:text-white font-sans overflow-hidden transition-colors duration-300">
      ...
      {/* SIDEBAR */}
      <aside
        className={`fixed md:static inset-y-0 left-0 z-30 flex w-[min(86vw,18rem)] flex-col border-r border-slate-200 bg-white transition-transform duration-300 dark:border-gray-800 dark:bg-[#1d1d20] md:w-64 ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}
      >
        ...
        <nav className="flex-1 space-y-2 overflow-y-auto px-3 py-3 sm:px-4 sm:py-4">
          {navItems.map((item) => (
            ...
            >
              ...
              {item.badge && (
                <span className="ml-auto bg-white/20 text-xs px-2 py-0.5 rounded-full font-bold">
                  {item.badge}
                </span>
              )}
            </Link>
        ...
        <div className="space-y-2 border-t border-slate-200 p-3 dark:border-gray-800 sm:p-4">
          {user.role === "ENTREPRENEUR" && (
            <Link
              to="/dashboard/create-pitch"
              className="w-full flex items-center justify-center gap-2 bg-primary py-3 px-4 rounded-xl text-black font-bold text-sm shadow-lg hover:brightness-110 transition-shadow"
            >
              <PlusCircle size={18} />
          ...
          <button
            ...
            className="w-full text-xs font-bold text-red-500 py-2 hover:bg-red-50 dark:hover:bg-red-900/10 rounded-lg"
          >
            Log Out
          </button>
        </div>
        ...
        <header className="sticky top-0 z-10 flex min-w-0 items-center justify-between gap-2 border-b border-slate-200 bg-white/90 px-3 py-3 backdrop-blur-md transition-colors duration-300 dark:border-gray-800 dark:bg-[#111113]/90 sm:px-5 md:px-6 md:py-4">
          <div className="flex min-w-0 items-center gap-2 md:hidden">
            <button
              ...
              className="rounded-lg border border-slate-200 bg-slate-100 p-2 dark:border-gray-800 dark:bg-[#1d1d20]"
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 truncate text-[10px] font-medium uppercase text-slate-500 dark:text-gray-400">
                {role}
              </p>
            </div>
          </div>
          ...
          <div className="ml-auto flex min-w-0 items-center gap-1.5 sm:gap-3 md:gap-4">
            ...
            >
              <div className="text-right hidden sm:block">
                ...
                <p className="text-[10px] text-slate-500 dark:text-gray-400 font-medium uppercase">
                  {role}
                </p>
              </div>
