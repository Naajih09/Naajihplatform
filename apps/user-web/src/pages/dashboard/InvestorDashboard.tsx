  ...
  <article className="flex flex-col rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition-colors hover:border-primary/40 dark:border-white/10 dark:bg-[#1d1d20]">
    <div className="mb-4 flex items-start justify-between gap-4">
      <div>
        ...
        <p className="mt-1 line-clamp-2 text-sm text-slate-500 dark:text-white/50">
          {pitch.tagline}
        </p>
      </div>
...
export default function InvestorDashboard() {
    ...
    const fetchNetwork = async () => {
      ...
      try {
        const [connectionsRes, partnersRes] = await Promise.all([
          fetch(`${API_BASE}/connections/user/${user.id}`, {
            headers: authHeaders,
          }),
  ...
  const savedPitches = pitches.filter((pitch) =>
    savedPitchIds.includes(pitch.id),
  );
        ...
        <header className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm text-slate-500 dark:text-white/60">
              Review approved opportunities, save promising pitches, and manage
              founder conversations from one place.
            </p>
          </div>
        ...
        <section className="grid grid-cols-1 gap-4 md:grid-cols-4">
          ...
          <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-[#1d1d20]">
            ...
            <p className="mt-4 text-2xl font-black">
              {avgTicket > 0 ? formatNaira(avgTicket) : "--"}
            </p>
        ...
        <section className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <aside className="space-y-6">
            <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-[#1d1d20]">
              <div className="mb-4 flex items-center justify-between">
                <h2 className="flex items-center gap-2 text-lg font-black">
                  <Building2 size={18} className="text-primary" /> My
                  Connections
                </h2>
                <Link
              ...
              {connections.length === 0 ? (
                <p className="text-sm text-slate-500 dark:text-white/50">
                  Accepted founder connections will appear here after requests
                  are approved.
                </p>
              ) : (
                <div className="space-y-3">
                  {connections.slice(0, 4).map((connection) => {
                    ...
                    return (
                      <div
                        key={connection.id}
                        className="rounded-lg border border-slate-100 p-3 dark:border-white/10"
                      >
                        <p className="font-bold">{getProfileName(contact)}</p>
                        <p className="text-xs text-slate-500 dark:text-white/50">
                          {contact?.entrepreneurProfile?.businessName ||
                            contact?.entrepreneurProfile?.industry ||
                            "Founder"}
                        </p>
                      </div>
                    );
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-white/10 dark:bg-[#1d1d20]">
              <div className="mb-4 flex items-center justify-between">
                <h2 className="flex items-center gap-2 text-lg font-black">
                  <MessageSquare size={18} className="text-primary" /> Recent
                  Messages
                </h2>
                <Link
              ...
              {chatPartners.length === 0 ? (
                <p className="text-sm text-slate-500 dark:text-white/50">
                  Message threads will appear after you connect and start a
                  conversation.
                </p>
              ) : (
                <div className="space-y-3">
            ...
            <div className="rounded-xl border border-primary/20 bg-primary/10 p-5">
              ...
              <p className="text-sm leading-6 text-slate-700 dark:text-white/80">
                Your dashboard prioritizes saved pitches, focus areas, and
                investment preference. Update those settings when your mandate
                changes.
              </p>
            </div>
          </aside>
