...
const AcademyDashboard = () => {
    ...
    const fetchProgram = async () => {
      ...
      try {
        ...
        if (data.modules?.length > 0) {
          const firstUnlocked = data.modules.find(
            (mod: any) => mod.isUnlocked !== false,
          );
  ...
  if (!program)
    return (
      <div className="text-center py-20 text-red-500">Program not found</div>
    );
  ...
  const activeModuleData = program.modules.find(
    (mod: any) => mod.id === activeModule,
  );
  ...
  const handleSubmitTask = async (taskId: string) => {
    ...
    if (!isEnrolled) {
      showToast(
        "Enrollment approval is required to submit assignments.",
        "error",
      );
      return;
    }
        ...
        <div className="relative z-10 flex flex-col md:flex-row justify-between md:items-end gap-6">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <span
                className={`text-[10px] font-bold uppercase px-3 py-1 rounded-full ${program.isPremium ? "bg-amber-500/20 text-amber-400" : "bg-green-500/20 text-green-400"}`}
              >
          ...
          <div className="w-full md:w-64">
            ...
            <div className="flex justify-between mt-4">
              <div className="text-center">
                ...
                <p className="text-[10px] text-slate-500 dark:text-gray-500 uppercase">
                  Lessons
                </p>
              </div>
              <div className="text-center">
          ...
          <div className="space-y-4">
            {program.modules.map((mod: any) => {
              ...
              const unlockDateLabel = mod.unlockDate
                ? new Date(mod.unlockDate).toLocaleDateString("en-NG", {
                    dateStyle: "medium",
                  })
                : null;
              ...
              return (
                <div
                  key={mod.id}
                  className={`border rounded-xl overflow-hidden transition-all ${mod.id === activeModule ? "bg-slate-50 dark:bg-[#151518] border-primary/50" : "bg-white dark:bg-[#1d1d20] border-slate-200 dark:border-gray-800"}`}
                >
                  {/* Module Header */}
                  <div
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`size-10 rounded-full flex items-center justify-center font-bold text-sm ${mod.id === activeModule ? "bg-primary text-black" : "bg-slate-200 dark:bg-gray-800 text-slate-500 dark:text-gray-500"}`}
                      >
                        {mod.order}
                      </div>
                      <div>
                        <h3
                          className={`font-bold ${mod.id === activeModule ? "text-slate-900 dark:text-white" : "text-slate-500 dark:text-gray-400"}`}
                        >
                          {mod.title}
                        </h3>
                    ...
                    {isLocked ? (
                      ...
                      <ChevronRight
                        size={20}
                        className={`transform transition-transform ${mod.id === activeModule ? "rotate-90" : ""} text-slate-500 dark:text-gray-500`}
                      />
                    )}
                  </div>
                        ...
                        return (
                          <div
                            ...
                            onClick={() => {
                              if (!isEnrolled) {
                                showToast(
                                  "Join the program to access lessons.",
                                  "error",
                                );
                                return;
                              }
                              ...
                              navigate(
                                `/dashboard/academy/course/${lesson.id}`,
                              );
                            }}
                          >
                            <div className="flex items-center gap-3">
                              ...
                              <span
                                className={`text-sm ${lesson.progress?.length > 0 ? "text-slate-500 dark:text-gray-500 line-through" : "text-slate-900 dark:text-white font-medium"}`}
                              >
                                {lesson.title}
                              </span>
                            </div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white dark:bg-[#1d1d20] border border-slate-200 dark:border-gray-800 rounded-xl p-6">
            <h3 className="text-lg font-bold mb-4 flex items-center gap-2">
              <AlertCircle className="text-yellow-500" size={20} /> Program
              Tasks
            </h3>
              ...
              {activeModuleData?.tasks?.length ? (
                activeModuleData.tasks.map((task: any) => {
                  ...
                  return (
                    <div
                      key={task.id}
                      className="bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-gray-800 rounded-xl p-4 space-y-3"
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div>
                          ...
                          {dueLabel && (
                            <p className="text-[10px] uppercase text-slate-500 dark:text-gray-500 mt-2">
                              Due {dueLabel}
                            </p>
                          )}
                        </div>
            ...
            {milestones.length ? (
              <div className="space-y-2">
                {milestones.map((entry: any) => (
                  <div
                    key={entry.id}
                    className="bg-slate-50 dark:bg-black/20 border border-slate-200 dark:border-gray-800 rounded-lg p-3"
                  >
                    ...
                    <p className="text-[10px] text-slate-500 dark:text-gray-500 mt-1">
                      Achieved{" "}
                      {new Date(entry.achievedAt).toLocaleDateString("en-NG", {
                        dateStyle: "medium",
                      })}
                    </p>
                  </div>
              ...
              <div className="text-[10px] text-slate-500 dark:text-gray-500 uppercase font-bold">
                <span className="inline-block rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-3 text-center text-sm text-slate-500 dark:border-white/10 dark:bg-white/[0.03] dark:text-gray-400">
                  No milestones yet. Complete lessons and assignments to unlock
                  progress markers here.
                </span>
              </div>
            )}
          </div>
            ...
            <div className="bg-primary/10 border border-primary/40 rounded-xl p-6">
              ...
              <Button
                ...
                onClick={() => {
                  if (!hasPremium) {
                    showToast(
                      "Premium access required for certificates.",
                      "error",
                    );
                    navigate("/dashboard/subscription");
                    return;
