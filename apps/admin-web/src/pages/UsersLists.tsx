...
const UsersList = () => {
  ...
  const handleDelete = async (id: string) => {
    if (
      !window.confirm(
        "Are you sure you want to BAN and DELETE this user? This cannot be undone.",
      )
    )
      return;
  ...
  const handleRoleUpdate = async () => {
    ...
    try {
      ...
      setUsers(
        users.map((u) =>
          u.id === viewUser.id ? { ...u, role: roleDraft } : u,
        ),
      );
  ...
  const hasFilters =
    searchQuery.trim() !== "" || roleFilter !== "ALL" || sortBy !== "date";
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
          ...
          <p className="text-slate-500 mt-1 dark:text-gray-500">
            Manage, verify, and monitor platform accounts.
          </p>
        </div>
            ...
            <table className="w-full text-left border-collapse min-w-[800px]">
              ...
              <tbody className="divide-y divide-slate-200 dark:divide-white/5 text-sm text-slate-700 dark:text-gray-300">
                {users.length === 0 ? (
                      ...
                      <EmptyState
                        ...
                        description={
                          hasFilters
                            ...
                            : "User accounts will appear here once people start signing up on the platform."
                        }
                        actionLabel={
                          hasFilters ? "Reset filters" : "Refresh users"
                        }
                        onAction={() => {
                          if (hasFilters) {
                            setSearchQuery("");
                  ...
                  users.map((user) => {
                    const profile =
                      user.entrepreneurProfile || user.investorProfile || {};
                      ...
                      >
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-3">
                            <div
                              className={`size-9 rounded-full flex items-center justify-center font-bold text-white ${isInvestor ? "bg-secondary" : "bg-primary text-black"}`}
                            >
                              {(profile.firstName ||
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${isInvestor ? "bg-purple-500/10 text-purple-400" : "bg-blue-500/10 text-blue-400"}`}
                          >
                            {user.role}
                          </span>
                        </td>
                        ...
                        <td className="px-6 py-4">
                          {user.isVerified ? (
                            ...
                            <span className="text-slate-500 dark:text-gray-500 flex items-center gap-1 text-xs">
          ...
          <div className="bg-white border border-slate-200 rounded-xl w-full max-w-lg p-6 relative dark:bg-[#1d1d20] dark:border-white/10">
            ...
            <div className="space-y-4 text-sm text-slate-700 dark:text-gray-300">
              <div className="grid grid-cols-2 gap-4 bg-slate-50 dark:bg-white/5 p-4 rounded-lg">
                <div>
                  <p className="text-slate-500 dark:text-gray-500 text-xs uppercase mb-1">
                    Email
                  </p>
                ...
                <div>
                  ...
                  <p
                    className={`font-medium ${viewUser.isActive === false ? "text-red-400" : "text-green-400"}`}
                  >
                ...
                <div>
                  ...
                  <p
                    className="font-mono text-xs text-slate-400 dark:text-gray-400 truncate"
                    title={viewUser.id}
                  >
                    {viewUser.id.substring(0, 12)}...
                  </p>
                </div>
              </div>
              ...
              {(viewUser.entrepreneurProfile || viewUser.investorProfile) && (
                <div className="bg-slate-50 dark:bg-white/5 p-4 rounded-lg space-y-3">
                  ...
                  {Object.entries(
                    ...
                    if (
                      ["id", "userId", "createdAt", "updatedAt"].includes(
                        key,
                      ) ||
                      !value
                    )
                    ...
                    return (
                      <div
                        key={key}
                        className="flex flex-col border-b border-slate-200 dark:border-white/5 pb-2"
                      >
                        <span className="text-slate-500 dark:text-gray-500 text-xs uppercase">
            ...
            <div className="mt-6 border-t border-slate-200 dark:border-white/10 pt-4 space-y-3">
              <div className="flex flex-col md:flex-row md:items-center gap-3">
                ...
                <label
                  htmlFor="update-role-select"
                  className="text-xs text-slate-500 dark:text-gray-500 uppercase font-bold"
                >
                  Update Role
                </label>
                <select
                ...
                >
                  ...
                  <option value="ASPIRING_BUSINESS_OWNER">
                    Aspiring Owner
                  </option>
