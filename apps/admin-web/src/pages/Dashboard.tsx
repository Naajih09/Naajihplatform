...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const usersResponse = dashboard.userStats;
        const statsResponse = dashboard.pitchStats;
        const verificationsResponse = dashboard.pendingVerifications;
        const auditResponse = dashboard.auditLogs;
        const insightsResponse = dashboard.insights;
  ...
  const handleAction = async (id: string, status: "APPROVED" | "REJECTED") => {
    if (
      !window.confirm(`Are you sure you want to ${status} this verification?`)
    )
      return;
    setActionLoading(id);
    try {
    ...
    <div className="max-w-7xl mx-auto space-y-8 pb-20">
                ...
                <tr>
                  <td colSpan={5} className="py-8 text-center text-gray-500">
                    <div className="mx-auto inline-flex flex-col items-center gap-3 rounded-xl border border-dashed border-slate-300 bg-slate-50 px-6 py-4 text-center text-sm text-slate-500 dark:border-white/10 dark:bg-white/[0.03] dark:text-gray-400">
                      <span>
                        No pending verifications. New KYC requests will appear
                        here.
                      </span>
                      <button
                        type="button"
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span className="bg-slate-100 dark:bg-white/10 px-2 py-1 rounded text-[10px] font-bold uppercase">
                          {req.user?.role}
                        </span>
                      </td>
                      <td className="px-6 py-4">
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-4 text-xs text-slate-500 dark:border-white/10 dark:bg-white/[0.03] dark:text-gray-400">
                <div>
                  No recent activity yet. Admin actions and system events will
                  appear here.
                </div>
                <button
                  type="button"
        ...
        <div className="rounded-xl border border-primary/20 bg-primary/5 p-6">
          ...
          <div className="space-y-4">
            {loading ? (
              ...
              <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-4 text-xs text-slate-500 dark:border-white/10 dark:bg-white/[0.03] dark:text-gray-400">
                <div>
                  No pitch categories yet. Category distribution will appear
                  after founders publish pitches.
                </div>
                <button
                  type="button"
      ...
      <div className="admin-surface overflow-hidden">
        <div className="p-6 border-b border-slate-200 dark:border-white/5">
          ...
          <p className="text-sm text-slate-500 dark:text-gray-500">
            Daily new users, pitches, and connections
          </p>
        </div>
        <div className="p-6">
          {loading ? (
            ...
            <div className="py-8 text-center text-gray-500">
              <div className="inline-flex flex-col items-center gap-3 rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-4 text-sm text-slate-500 dark:border-white/10 dark:bg-white/[0.03] dark:text-gray-400">
                <span>
                  No activity data yet. This chart will populate as platform
                  usage increases.
                </span>
                <button
                  type="button"
...
const formatAuditTitle = (log: any) => {
  if (log.action === "VERIFICATION_STATUS_UPDATED") {
    ...
    const target =
      log.metadata?.targetUserEmail || log.metadata?.targetUserId || "user";
  ...
  <div className="p-6 rounded-xl border border-slate-200 bg-white flex flex-col justify-between min-h-[140px] shadow-sm dark:border-white/5 dark:bg-gradient-to-br dark:from-[#262626] dark:to-[#1a1a1a]">
    ...
    <div className="mt-4">
      ...
      <div className="flex items-center gap-1 mt-1">
        ...
        <span className="text-slate-500 dark:text-gray-500 text-[10px] uppercase">
          {subLabel}
        </span>
      </div>
    </div>
